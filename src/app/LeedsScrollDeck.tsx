    ...
    >
      ...
      <main className="px-6">
        {DATA.sections.map((s, idx) => (
            ...
            {s.table && (
              <motion.div 
                className="mt-8 overflow-hidden border border-black/10"
              ...
              >
                <table className="w-full text-left">
                  ...
                  >
                    <tr>
                      {s.table.headers.map((h: string, idx: number) => (
                        <motion.th 
                          key={h} 
                          className="px-6 py-4 text-xs uppercase tracking-[0.15em] font-semibold text-black/50"
                  ...
                  <tbody>
                    {s.table.rows.map((row: string[], i: number) => (
                      <motion.tr 
                        key={i} 
                        className="border-t border-black/10 hover:bg-black/1 transition-colors"
                      ...
                      >
                        {row.map((cell: string, j: number) => (
                          <td key={j} className="px-6 py-5 text-base text-black/80">{cell}</td>
                        ))}
                      </motion.tr>
                    ))}
                  </tbody>
            ...
            {s.roadmap && (
              <div className="mt-8 grid md:grid-cols-2 gap-6">
                {s.roadmap.map((phase, i) => (
                  <motion.div 
                    key={i} 
                    className="border border-black/10 p-6 hover:border-black/30 transition-colors group relative overflow-hidden"
                    initial={{ 
                      opacity: 0, 
                      y: 50,
                      rotateY: i % 2 === 0 ? -15 : 15,
                  ...
                  >
                    <motion.div
                      className="absolute top-0 right-0 w-20 h-20 bg-black/5 rounded-full -translate-y-10 translate-x-10"
                      initial={{ scale: 0 }}
                      whileInView={{ scale: 1 }}
                      viewport={{ once: false, margin: "-20%" }}
                      transition={{ duration: 0.6, delay: 0.5 + i * 0.12 }}
                    />
                    <h3 className="text-lg font-semibold mb-4 text-black group-hover:text-black/80 transition-colors relative z-10">{phase.phase}</h3>
                    <ul className="space-y-3 relative z-10">
                      {phase.items.map((it: string, j: number) => (
                        ...
                        >
                          <motion.span 
                            className="text-black/20 font-mono text-xs mt-1"
                            whileHover={{ 
            ...
            {s.metrics && (
              <div className="mt-8 grid md:grid-cols-2 gap-6">
                {s.metrics.map((m, i) => (
                  <motion.div 
                    key={i} 
                    className="border-l-2 border-black pl-6 py-4 hover:border-black/60 transition-colors group relative"
                    initial={{ 
                      opacity: 0, 
                      x: -50,
                      borderLeftWidth: 0
                  ...
                  >
                    <motion.div 
                      className="text-2xl font-bold text-black mb-2 group-hover:text-black/80 transition-colors"
            ...
            {s.callout && (
              <motion.div 
                className="mt-8 p-8 bg-black text-white text-xl font-medium leading-relaxed relative overflow-hidden"
